...
export function IronMountain({ state }: { state: MountainState }) {
  ...
  const from =
    state.lastReachedIndex >= 0 ? WP_POS[state.lastReachedIndex] : START;
  const to = WP_POS[Math.min(state.lastReachedIndex + 1, WP_POS.length - 1)];
  const t =
    state.lastReachedIndex >= WP_POS.length - 1 ? 1 : state.progressToNext;
  const flag = {
    x: from.x + (to.x - from.x) * t,
    y: from.y + (to.y - from.y) * t,
  };

  const pathD = [START, ...WP_POS]
  ...
  return (
    <Card
      ...
      accent={
        <Pill className="!text-glow-cyan">
          {Math.round(state.pctToSummit * 1000) / 10}% to the summit
        </Pill>
      }
    >
      <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-b from-[#0c1226] to-[#131a33]">
        <svg
          ...
          role="img"
          aria-label="Your climb up Iron Mountain"
        >
          {/* stars */}
          {[
            [60, 40],
          ...
          {/* snow cap */}
          <polygon
            points="616,53 652,36 686,58 668,62 652,52 636,64"
            fill="#dbe6f5"
            opacity={0.9}
          />
            ...
            return (
              <g key={w.name}>
                <title>
                  ...
                  {w.reached && w.reachedDate
                    ? ` · reached ${shortDate(w.reachedDate)}`
                    : ""}
                </title>
                <circle
                  cx={p.x}
          ...
          <g>
            ...
            />
            <path
              d={`M${flag.x} ${flag.y - 20} L${flag.x + 14} ${flag.y - 15} L${flag.x} ${flag.y - 10} Z`}
              fill="#8b5cf6"
            />
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        {state.nextName && (
          <p className="text-sm text-slate-400">
            Next camp: <span className="text-slate-200">{state.nextName}</span>{" "}
            in{" "}
            <span className="font-medium text-glow-emerald">
              {fmtWeight(state.kgToNext, unit)}
            </span>
          </p>
        )}
      </div>
